...
func TestConstantsErrorsMessages(t *testing.T) {
	assert.EqualValues(t, "error while cloning repository", ErrMsgCloneFail)
	...
	assert.EqualValues(t, "error while getting tags due to %s", ErrMsgGetingTags)
	assert.EqualValues(t, "error while creating tag", ErrMsgCreateTag)
	assert.EqualValues(t, "tag %s already exists", ErrMsgTagExists)
	assert.EqualValues(t, "error while iterating tags due to: %s", ErrMsgIterateTagsError)
	assert.EqualValues(t, "tag %s does not exists", ErrMsgTagDoesNotExists)
	assert.EqualValues(t, "error while casting interface to struct", ErrMsgInterfaceToStruct)
	assert.EqualValues(t, "error while getting public key", ErrMsgGettingPublicKey)
	assert.EqualValues(t, "error while adding changes to stage", ErrMsgAddingToStage)
	assert.EqualValues(t, "error while commiting changes to tranfer area", ErrMsgCommitingToTransfer)
	assert.EqualValues(t, "error while pushing tags", ErrMsgPushTags)
	assert.EqualValues(t, "error while pushing commits", ErrMsgPushCommits)
}
